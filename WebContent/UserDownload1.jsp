...
<body style="margin:0 auto;width:1000px">
		...
		<nav class="navbar navbar-expand-lg navbar-light bg-faded">
		  ...
		  <div class="collapse navbar-collapse justify-content-center" id="navbarNavDropdown">
			<ul class="navbar-nav">
			  ...
			  <li class="nav-item">
				<a class="nav-link" href="RequestKey.jsp">REQUEST KEY</a>
			  </li>
			  <li class="nav-item">
	...
	try
	{
		...
		ResultSet rs = st.executeQuery("SELECT * FROM filerequest WHERE FILEID='"+fid+"' AND UID='"+uid+"'");
		if(rs.next())
		{
			%>
<h2 style="padding-top:30px;padding-left:60px;font-family:Trebuchet MS, Helvetica, sans-serif;font-weight: bold;color:white;">ENTERED FILE DETAILS</h2>
			
		<form method="post" action="UserDownload2.jsp">
		<table id="customers" style="margin-left:60px;">
		<tr><th>FILE ID</th><td><input type="text" name="fid" required="" value=<%=fid %> readonly></td></tr>
		<tr><th>NAME</th><td><input type="text" name="fname" required="" value=<%=rs.getString("FILENAME") %> readonly></td></tr>	
		<tr><th>DATAOWNER NAME</th><td><input type="text" name="oname" required="" value=<%=rs.getString("OWNERNAME")%> readonly></td></tr>	
		<%
		Statement st1 = con.createStatement();
		ResultSet rs1 = st1.executeQuery("SELECT * FROM upload WHERE FID='"+fid+"'");
		if(rs1.next())
		{
			enc = rs1.getString("ENC");
			 %>
			<tr><th>ENCRTYPTED CONTENT</th><td><textarea rows="10" cols="40" readonly><%=enc%></textarea></td></tr>	
			<%
		}
		%>
		<tr><th>DECRYPTION KEY</th><td><input type="password" name="dkey" required="" id="myInput" placeholder="Enter Key"><br><input id="check3" type="checkbox" onclick="myFunction()">&nbsp;&nbsp;<label style="color:white;">Show DecryptionKey</label></td></tr>
		...
		<tr><th></th><td><input class="button" type="submit" VALUE="DECRYPT"></td></tr>
		</table>
		</form>			
			<%
		}
